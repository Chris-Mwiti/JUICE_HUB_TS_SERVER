...
function logger(loggerLabel:string){
    return winston.createLogger({
        ...
        transports:[
            ...
            new transports.File({filename: pathRedirector("errLogs.txt"),level:'error'}),
            new transports.File({filename: pathRedirector("warnLogs.txt"), level: 'warn'}),
            new transports.File({filename: pathRedirector("infoLogs.txt"),level: 'info' })
        ]
    })
}
